...
public class ConsultarCargaProfesorControllerFX implements Initializable {
    ...
    public void onActionConsultarCarga(ActionEvent event) {
        ...
        try {
            ...
            // URL del backend
            URL url = new URL("http://localhost:8080/api/directordepartamento/erprofesor?pId=" + id);
            ...
            if (responseCode == HttpURLConnection.HTTP_OK) {
                ...
                mostrarAlertaInformacion(
                        "Informe generado",
                        "El informe del profesor ha sido generado.\nRecuerda que aparecerá un diálogo para guardarlo."
                );

            } else {
                mostrarAlertaError(
                        "Error en el servidor",
                        "No se pudo generar el informe. Código HTTP: " + responseCode
                );
            }
            ...
            mostrarAlertaError("Error de conexión", "No se pudo contactar al servidor.\n" + e.getMessage());
        }
    }
